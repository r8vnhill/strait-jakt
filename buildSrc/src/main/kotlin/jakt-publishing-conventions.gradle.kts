...
signing {
   // Configure GPG signing if the signing key and password are provided.
   if (!signingKey.isNullOrBlank() && !signingPassword.isNullOrBlank()) {
      useGpgCmd()
      useInMemoryPgpKeys(signingKey, signingPassword)
   }
   // Sign all publications defined in the publishing configuration.
   sign(publishing.publications)
   // Set signing as required only for release versions.
